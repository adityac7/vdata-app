...
const tools: Tool[] = [
  ...
  {
    name: "run_query",
    description: "Execute SELECT query. DATABASES: 'main'=digital_insights (app usage/duration/demos - type,cat,genre,age_bucket,gender,nccs_class,state_grp,day_of_week,population,app_name,duration_sum,event_id,user_id) | 'hul'=hul_combined_data (ecom: vtionid,app,brand,event_ad,event_search,event_product,event_cart,cat,parent_company,\"Seen Ad\",\"Searched Product\",\"Seen Product\",\"Added to Cart\",\"Category Interaction\",Month,Year,nccs_code,age,gender,State,Population,zone - 629K rows). ARRAY COLUMNS in hul (use string_to_array(col,',') & UNNEST): app,\"Seen Ad\",\"Searched Product\",\"Seen Product\",\"Added to Cart\",\"Category Interaction\". CONTEXT: FMCG ecom analyst for India using consented Android panel (~850 cells age×gender×NCCS×townclass×state). WEIGHTING: every weight unit = 1,000 people; population metrics = SUM(metric*weight*1000) with NCCS merged (A/A1→A, B→B, C/D/E→CDE) and event_date >= NOW()-2 years. TIME: default base last month, trend trailing 3 months; funnel order ads→search→pdp/category→cart. OUTPUT: precise markdown tables + p<0.05 notes, terse commentary. Choose DB by query intent or join both as needed.",
    inputSchema: {
      type: "object",
      properties: {
        query: {
